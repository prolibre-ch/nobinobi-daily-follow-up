...
def coverage(c):
    ...
    """
    c.run("coverage run --source nobinobi-daily-follow-up runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/nobinobi-daily-follow-up.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ nobinobi_daily_follow_up")
...
def lint(c):
    ...
    """
    c.run("flake8 nobinobi-daily-follow-up tests")
...
def release(c, bumpsize=''):
    ...
    import nobinobi_daily_follow_up
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=nobinobi_daily_follow_up.__version__))
    c.run("git push --tags")
